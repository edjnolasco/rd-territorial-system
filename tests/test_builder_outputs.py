...
def test_build_v2_0_outputs_reports(tmp_path, monkeypatch) -> None:
    ...
    adm1 = {
        ...
        "features": [
            {
                "type": "Feature",
                "properties": {"NAME_1": "Distrito Nacional", "ID_1": "RD-01"},
                "geometry": None,
            },
            {
                "type": "Feature",
                "properties": {"NAME_1": "Santo Domingo", "ID_1": "RD-32"},
                "geometry": None,
            },
        ],
    }
        ...
        "features": [
            {
                "type": "Feature",
                "properties": {
                    "NAME_1": "Distrito Nacional",
                    "NAME_2": "Santo Domingo de Guzman",
                    "ID_1": "RD-01",
                    "ID_2": "RD-01-01",
                },
                "geometry": None,
    ...
    monkeypatch.setattr(
        "rd_territorial_system.builder.PROVINCES_OUTPUT", out_dir / "provinces.geojson"
    )
    monkeypatch.setattr(
        "rd_territorial_system.builder.MUNICIPALITIES_OUTPUT", out_dir / "municipalities.geojson"
    )
    monkeypatch.setattr(
        "rd_territorial_system.builder.MASTER_OUTPUT", out_dir / "territorial_master.csv"
    )
    monkeypatch.setattr(
        "rd_territorial_system.builder.MATCH_REPORT_OUTPUT", out_dir / "match_report.csv"
    )
    monkeypatch.setattr(
        "rd_territorial_system.builder.COVERAGE_REPORT_OUTPUT", out_dir / "coverage_report.csv"
    )
    monkeypatch.setattr(
        "rd_territorial_system.builder.INGESTION_REPORT_OUTPUT", out_dir / "ingestion_report.json"
    )
    monkeypatch.setattr(
        "rd_territorial_system.builder.UNMATCHED_MUNICIPALITIES_OUTPUT",
        out_dir / "unmatched_municipalities.csv",
    )
    monkeypatch.setattr(
        "rd_territorial_system.builder.LOW_CONFIDENCE_OUTPUT",
        out_dir / "low_confidence_matches.csv",
    )
