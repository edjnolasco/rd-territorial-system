...
def build_coverage_report(master_df: pd.DataFrame) -> pd.DataFrame:
    ...
    matched_municipality = (
        int(master_df["matched_municipality"].fillna(False).sum()) if total else 0
    )
    ...
    unmatched_count = int((~master_df["matched_municipality"]).sum()) if total else 0
    ...
    rows = [
        ...
        {
            "metric": "province_match_rate",
            "value": round(matched_province / total, 4) if total else 0,
        },
        {
            "metric": "municipality_match_rate",
            "value": round(matched_municipality / total, 4) if total else 0,
        },
    ]
